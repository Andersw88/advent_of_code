...
points = deque(zip(input_array, range(1, len(input_array) + 1), [0] * len(input_array)))
...
while(points):
    ...
    if point[0] + 1 <= max_x:
        proccess_point(grid, points, (point[0] + 1, point[1]), curr_index, curr_dist)

    if point[1] + 1 <= max_y:
        proccess_point(grid, points, (point[0], point[1] + 1), curr_index, curr_dist)
