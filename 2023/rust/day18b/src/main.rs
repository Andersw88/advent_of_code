...
fn main() {
    ...
    input.for_each(|line| {
        ...
        match hex.chars().nth_back(1).unwrap() {
            '1' => { current_pos = (current_pos.0, current_pos.1 + length)},
            '3' => { current_pos = (current_pos.0, current_pos.1 - length)},
            '2' => { current_pos = (current_pos.0 - length, current_pos.1)},
            '0' => { current_pos = (current_pos.0 + length, current_pos.1)}
            _ => unreachable!(),
        }
    ...
    let result = instructions.windows(2).fold(0, |area ,b| {
        let b0 = b[0];
        let b1 = b[1];
        area + (b0.0) * (b1.1) - (b0.1) * (b1.0) + (i64::abs(b0.0 - b1.0) + i64::abs(b0.1 - b1.1))
    }) / 2;
