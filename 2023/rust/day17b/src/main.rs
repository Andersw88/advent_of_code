...
fn main() {
    ...
    // let input = include_str!("example2.txt").lines();
    ...
    open_list.push(Node::new(0, 0, (0, 1)).into());
    ...
    while let Some(node) = open_list.pop() {
        ...
        if node.x == goal.0 && node.y == goal.1 && node.steps >= 4{
            result = Some(node.clone());
            break;
        }

        let new_nodes: Vec<Rc<Node>> = directions
            .iter()
            .filter_map(|dir| {

                if node.steps < 4 && dir != &node.direction
                {
                    return None;
                }
                ...
                if nx >= 0 && nx < width as i32 && ny >= 0 && ny < height as i32 && new_steps <= 10 {
