...
fn main() {
    ...
    (0..cards.len())
        .for_each(|i| {
            ...
            (i + 1..=i + wins).for_each(|j| {
                if j < cards.len()
                {
                    cards[j].copies += card_copies
                }
            });
        });
    ...
    for card in &cards {
        println!(
            "Card copies {}: Winning Numbers: {:?}, Your Numbers: {:?}",
            card.copies, card.winning_numbers, card.your_numbers
        );
    }

    let result: i64 = cards.iter().map(|card| card.copies).sum();
